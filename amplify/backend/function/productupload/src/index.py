...
def parse_sheet_good(row, supplier_id):
    ...
    concatenated_id = f"Sheet Good{length}#{width}#{thickness}#{species}#{grade}#{panel_type}#{treatment}#{edge}#{finish}#{brand}#{origin}#{metric}"
    ...
    if "packSize" in row and row['packSize'] != '':
        ...
        pack_size = BUNDLE_SIZES["Sheet Good"].get(thickness)
...
def handler(event, context):
    ...
    if category and category not in ['Lumber', 'Sheet Good']:
        return {
            'statusCode': 400,
            'body': json.dumps({
